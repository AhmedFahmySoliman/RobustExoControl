...
warning('off','all');
[h(1),p(1)] = lillietest(rep_outlier(T_F.Xzmp_ZIF)); [h(2),p(2)] = lillietest(rep_outlier(T_F.Yzmp_ZIF)); [h(3),p(3)] = lillietest(rep_outlier(T_F.Pitch_ZIF)); 
[h(4),p(4)] = lillietest(rep_outlier(T_F.Xzmp_BAC)); [h(5),p(5)] = lillietest(rep_outlier(T_F.Yzmp_BAC)); [h(6),p(6)] = lillietest(rep_outlier(T_F.Pitch_BAC));
[h(7),p(7)] = lillietest(rep_outlier(T_F.Xzmp_CMC)); [h(8),p(8)] = lillietest(rep_outlier(T_F.Yzmp_CMC)); [h(9),p(9)] = lillietest(rep_outlier(T_F.Pitch_CMC));
...
%% Compute ANOVA test
rm = fitrm(T_F,'Xzmp_ZIF-Pitch_CMC ~ 1','WithinDesign',within_zmp,'WithinModel','orthogonalcontrasts');
ANOVA_SC1 = ranova(rm,'WithinModel','Controller');
disp('ANOVA Table1 =')
disp(ANOVA_SC1)

%% PairWise Compare
%Post_Hoc_SC1 = multcompare(rm,'Controller');
%disp('Post-Hoc Table1 =')
%disp(Post_Hoc_SC1)
